...
class Insights:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "top_entities": sorted(self.top_entities, key=lambda x: (-x.get("count", 0), x.get("type", ""))),
...
class WXPResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return _sort_dict({
            ...
            "chunks": sorted([c.to_dict() for c in self.chunks], key=lambda x: x.get("index", 0)),
            "entities": sorted([e.to_dict() for e in self.entities], key=lambda x: (x.get("type", ""), x.get("value", ""))),
            "relations": sorted([r.to_dict() for r in self.relations], key=lambda x: (x.get("source", ""), x.get("target", ""))),
