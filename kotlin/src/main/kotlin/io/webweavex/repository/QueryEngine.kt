...
object QueryEngine {
    ...
    fun booleanQuery(graph: KnowledgeGraph, must: List<String> = emptyList(), should: List<String> = emptyList(), mustNot: List<String> = emptyList()): QueryResult {
        val mustMatch = mutableSetOf<String>(); val shouldMatch = mutableSetOf<String>(); val mustNotMatch = mutableSetOf<String>()
        ...
        val matches: List<Map<String, Any>> = graph.nodes.filter { it.id in matchedIds }.sortedBy { it.id }.map { mapOf("id" to it.id, "type" to it.type, "data" to it.data) }
        return QueryResult(matches, matches.size, graph.fingerprint(), matches.map { it["id"] as String })
    }
    ...
    private fun computeScore(node: KnowledgeNode, query: String): Double {
        ...
        for (v in node.data.values) { val vs = v.toString(); if (vs.contains(query, ignoreCase = true)) score += if (vs.equals(query, ignoreCase = true)) 8.0 else 3.0 }
        return score
    }
}
